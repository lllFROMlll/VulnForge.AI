...
class PortScanScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        targetField = findViewById(R.id.port_scan_target)
        resultText = findViewById(R.id.port_scan_result)

        findViewById<Button>(R.id.port_scan_quick).setOnClickListener {
            ...
            runCommand("nmap -p 80,443,22,8080 -sV $target")
        }

        findViewById<Button>(R.id.port_scan_full).setOnClickListener {
            val target = targetField.text.toString().trim()
            if (target.isEmpty()) { resultText.text = "Digite um IP ou domínio."; return@setOnClickListener }
            runCommand("nmap -p- -sV $target")
        }
    }
